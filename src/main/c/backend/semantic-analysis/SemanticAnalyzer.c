...
static void _validatePalettes(PaletteEntry *palette, CompilerState *state) {
  while (palette) {
    ...
    if (palette->paletteName) {
      ...
      if (!addPaletteColor(state->symbolTable, palette->paletteName,
...
static void _validateFigureProperties(Figure *figure, CompilerState *state) {
  ...
  while (prop) {
    ...
    case WIDTH_PROPERTY:
      ...
      if (figure->type == CIRCLE_FIGURE) {
        state->isValid = false;
        state->errorMessage =
            "Property 'width' is not valid for circle figures.";
        return;
      } else if (figure->type == LINE_FIGURE ||
                 figure->type == POLYLINE_FIGURE ||
                 figure->type == POLYGON_FIGURE) {
        state->isValid = false;
        state->errorMessage =
            "Property 'width' is not valid for line/polyline/polygon figures.";
        return;
      }
      break;
    case HEIGHT_PROPERTY:
      ...
      if (figure->type == CIRCLE_FIGURE) {
        state->isValid = false;
        state->errorMessage =
            "Property 'height' is not valid for circle figures.";
        return;
      } else if (figure->type == LINE_FIGURE ||
                 figure->type == POLYLINE_FIGURE ||
                 figure->type == POLYGON_FIGURE) {
        state->isValid = false;
        state->errorMessage =
            "Property 'height' is not valid for line/polyline/polygon figures.";
        return;
      }
      break;
    case SIZE_PROPERTY:
      ...
      if (figure->type == CIRCLE_FIGURE) {
        state->isValid = false;
        state->errorMessage =
            "Property 'size' is not valid for circle figures.";
        return;
      } else if (figure->type == LINE_FIGURE ||
                 figure->type == POLYLINE_FIGURE ||
                 figure->type == POLYGON_FIGURE) {
        state->isValid = false;
        state->errorMessage =
            "Property 'size' is not valid for line/polyline/polygon figures.";
        return;
      }
      break;
    case RADIUS_PROPERTY:
      ...
      if (figure->type == RECTANGLE_FIGURE) {
        state->isValid = false;
        state->errorMessage =
            "Property 'radius' is not valid for rectangle figures.";
        return;
      } else if (figure->type == LINE_FIGURE ||
                 figure->type == POLYLINE_FIGURE ||
                 figure->type == POLYGON_FIGURE) {
        state->isValid = false;
        state->errorMessage =
            "Property 'radius' is not valid for line/polyline/polygon figures.";
        return;
      }
      break;
    default:
  ...
  if (hasSize && (hasWidth || hasHeight)) {
    state->isValid = false;
    state->errorMessage =
        "Cannot use both 'size' and 'width'/'height' properties.";
    return;
  }
  ...
  case RECTANGLE_FIGURE:
    if (!hasSize && !hasWidth && !hasHeight) {
      state->isValid = false;
      state->errorMessage =
          "Rectangle must have 'size' or both 'width' and 'height'.";
    } else if (!hasSize && (hasWidth != hasHeight)) {
      state->isValid = false;
      state->errorMessage =
          "Must specify both 'width' and 'height' when not using 'size'.";
    }
    break;
  case ELLIPSE_FIGURE:
    if (!hasSize && !hasWidth && !hasHeight) {
      state->isValid = false;
      state->errorMessage =
          "Ellipse must have 'size' or both 'width' and 'height' (rx/ry).";
    } else if (!hasSize && (hasWidth != hasHeight)) {
      state->isValid = false;
      state->errorMessage =
          "Must specify both 'width' and 'height' when not using 'size'.";
  ...
  while (properties) {
    ...
    if (properties->isExpression && properties->value.expressionValue) {
      ComputationResult result =
          computeExpression(properties->value.expressionValue, variables);
      if (!result.succeeded) {
        state->isValid = false;
        state->errorMessage =
            "Failed to evaluate expression (e.g., division by zero).";
        return;
      }
    ...
    switch (properties->type) {
    case OPACITY_PROPERTY:
      if (properties->value.floatValue < 0.0f ||
          properties->value.floatValue > 1.0f) {
        state->isValid = false;
