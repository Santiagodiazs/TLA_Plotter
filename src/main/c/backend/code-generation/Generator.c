...
static void _generateFigure(const unsigned int indentationLevel,
                            Figure *figure);
static void _generateFigures(const unsigned int indentationLevel,
                             Figure *figures);
static void _generateProperties(const unsigned int indentationLevel,
                                Property *properties, FigureType figureType);
static void _generateSymbols(const unsigned int indentationLevel, Scene *scene);
static void _generateUseInstances(const unsigned int indentationLevel,
                                  Scene *scene);
static void _generateTransforms(const unsigned int indentationLevel,
...
static void _generateGroupProperties(const unsigned int indentationLevel,
                                     Property *properties);
 ...
 */
static void _generateEpilogue(void) { fprintf(f, "</svg>\n"); }
...
static void _generateScene(const unsigned int indentationLevel, Scene *scene) {
  ...
  RenderableItem *items =
      (RenderableItem *)malloc(sizeof(RenderableItem) * count);
  int idx = 0;
  ...
  for (int i = 0; i < count; ++i) {
    if (items[i].type == ITEM_FIGURE) {
      ...
      // UseInstance behaves like a Rectangle (x, y, width, height) for
      // properties
 ...
 */
static void _generateLayers(const unsigned int indentationLevel,
                            Layer *layers) {
...
static void _generateGroupProperties(const unsigned int indentationLevel,
  ...
  while (prop) {
    if (prop->type == SCALE_PROPERTY || prop->type == ROTATE_PROPERTY ||
        prop->type == TRANSLATE_PROPERTY) {
      hasTransform = true;
      break;
    }
    prop = prop->next;
  ...
  if (hasTransform) {
    ...
    while (prop) {
      ...
      case TRANSLATE_PROPERTY:
        if (prop->isExpression && (prop->value.coordinates.xExpression ||
                                   prop->value.coordinates.yExpression)) {
          int tx = 0, ty = 0;
          if (prop->value.coordinates.xExpression) {
            ComputationResult r = computeExpression(
                prop->value.coordinates.xExpression, _currentVariables);
            if (r.succeeded)
              tx = r.value;
          }
          if (prop->value.coordinates.yExpression) {
            ComputationResult r = computeExpression(
                prop->value.coordinates.yExpression, _currentVariables);
            if (r.succeeded)
              ty = r.value;
          }
          _output(0, "translate(%d, %d) ", tx, ty);
        } else {
          _output(0, "translate(%d, %d) ", prop->value.coordinates.x,
                  prop->value.coordinates.y);
        }
        break;
      case ROTATE_PROPERTY:
        if (prop->isExpression && prop->value.expressionValue) {
          ComputationResult r =
              computeExpression(prop->value.expressionValue, _currentVariables);
          if (r.succeeded)
            _output(0, "rotate(%d) ", r.value);
        } else {
...
static void _generateProperties(const unsigned int indentationLevel,
  ...
  while (properties) {
    ...
    case POSITION_PROPERTY: {
      ...
      if (properties->isExpression) {
        if (properties->value.coordinates.xExpression) {
          ComputationResult r = computeExpression(
              properties->value.coordinates.xExpression, _currentVariables);
          if (r.succeeded)
            x = r.value;
        }
        if (properties->value.coordinates.yExpression) {
          ComputationResult r = computeExpression(
              properties->value.coordinates.yExpression, _currentVariables);
          if (r.succeeded)
            y = r.value;
        }
      }
    ...
    case SIZE_PROPERTY:
      ...
      if (figureType == ELLIPSE_FIGURE) {
        _output(0, " rx=\"%d%s\" ry=\"%d%s\"",
                properties->value.dimensions.width / 2,
                _unitToString(properties->value.dimensions.widthUnit),
                properties->value.dimensions.height / 2,
                _unitToString(properties->value.dimensions.heightUnit));
      } else {
        _output(0, " width=\"%d%s\" height=\"%d%s\"",
                properties->value.dimensions.width,
                _unitToString(properties->value.dimensions.widthUnit),
                properties->value.dimensions.height,
                _unitToString(properties->value.dimensions.heightUnit));
      }
      break;
    case WIDTH_PROPERTY: {
      ...
      if (properties->isExpression && properties->value.expressionValue) {
        ComputationResult r = computeExpression(
            properties->value.expressionValue, _currentVariables);
        if (r.succeeded)
          w = r.value;
      }
      if (figureType == ELLIPSE_FIGURE) {
        _output(0, " rx=\"%d%s\"", w / 2,
                _unitToString(properties->value.dimensions.widthUnit));
      } else {
        _output(0, " width=\"%d%s\"", w,
                _unitToString(properties->value.dimensions.widthUnit));
      }
      break;
    }
    case HEIGHT_PROPERTY: {
      ...
      if (figureType == ELLIPSE_FIGURE) {
        _output(0, " ry=\"%d%s\"", h / 2,
                _unitToString(properties->value.dimensions.heightUnit));
      } else {
        _output(0, " height=\"%d%s\"", h,
                _unitToString(properties->value.dimensions.heightUnit));
      }
      break;
    }
    ...
    case FILL_PROPERTY:
      ...
      if (properties->value.colorValue->type == NAMED_COLOR) {
        ...
        _output(0, "rgb(%d,%d,%d)", properties->value.colorValue->value.rgb.r,
                properties->value.colorValue->value.rgb.g,
                properties->value.colorValue->value.rgb.b);
      } else if (properties->value.colorValue->type == RGBA_COLOR_TYPE) {
        _output(0, "rgba(%d,%d,%d,%.2f)",
                properties->value.colorValue->value.rgba.r,
                properties->value.colorValue->value.rgba.g,
                properties->value.colorValue->value.rgba.b,
                properties->value.colorValue->value.rgba.a);
      }
      _output(0, "\"");
      break;
 ...
 */
static void _generateSymbols(const unsigned int indentationLevel,
                             Scene *scene) {
  if (!scene->symbols)
    return;
    ...
    case TRANSFORM_TRANSLATE:
      if (transforms->isExpression) {
        ...
        if (transforms->value.translate.txExp) {
          ComputationResult r = computeExpression(
              transforms->value.translate.txExp, _currentVariables);
          if (r.succeeded)
            tx = r.value;
        }
        if (transforms->value.translate.tyExp) {
          ComputationResult r = computeExpression(
              transforms->value.translate.tyExp, _currentVariables);
          if (r.succeeded)
            ty = r.value;
        }
        _output(0, "translate(%d, %d) ", tx, ty);
      } else {
        _output(0, "translate(%.2f, %.2f) ", transforms->value.translate.tx,
                transforms->value.translate.ty);
      }
      break;
    case TRANSFORM_ROTATE:
      if (transforms->isExpression) {
        ...
        if (transforms->value.rotate.degreesExp) {
          ComputationResult r = computeExpression(
              transforms->value.rotate.degreesExp, _currentVariables);
          if (r.succeeded)
            deg = r.value;
        }
    ...
    case TRANSFORM_SCALE:
      if (transforms->isExpression) {
        ...
        if (transforms->value.scale.sxExp) {
          ComputationResult r = computeExpression(transforms->value.scale.sxExp,
                                                  _currentVariables);
          if (r.succeeded)
            s = r.value;
        }
 ...
 */
static void _generateProgram(Program *program) {
  if (program == NULL || program->scene == NULL)
    return;
...
static void _generatePrologue(void) {
  ...
  fprintf(f, "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"800\" "
             "height=\"600\">\n");
}
...
static void _output(const unsigned int indentationLevel,
  ...
  vfprintf(f, effectiveFormat,
           arguments); // Write to file 'f' instead of stdout
  fflush(f);
...
void executeGenerator(CompilerState *compilerState) {
  ...
  logDebugging(_logger,
               "Generation is done. File scene.svg created successfully.");
}
