...
extern void flexEnterContext(LexicalAnalyzer *lexicalAnalyzer,
...
InputBuffer *createInputBuffer(LexicalAnalyzer *lexicalAnalyzer,
                               const char *path) {
  ...
  inputBuffer->buffer =
      yy_create_buffer(inputBuffer->file, inputBuffer->bufferSizeInBytes,
...
LexicalAnalyzer *createLexicalAnalyzer() {
  LexicalAnalyzer *lexicalAnalyzer =
      (LexicalAnalyzer *)calloc(1, sizeof(LexicalAnalyzer));
...
void destroyToken(Token *token) {
  if (token != NULL) {
    ...
    if (token->semanticValue != NULL) {
      ...
      // La cadena será liberada al destruir el AST (en destroyProperty,
      // destroyFigure, etc.)
...
void enterLexicalAnalyzerContext(LexicalAnalyzer *lexicalAnalyzer,
                                 FlexContext flexContext) {
  flexEnterContext(lexicalAnalyzer, flexContext);
}

CompilationStatus executeLexicalAnalysis(LexicalAnalyzer *lexicalAnalyzer) {
  return (CompilationStatus)yylex(NULL, (YYLTYPE *)lexicalAnalyzer->location,
                                  lexicalAnalyzer->scanner);
}
...
CompilationStatus executeSyntacticAnalysis() {
  ...
  logDebugging(_logger, "Compilation status: %s.",
               _compilationStatusAsString(status));
...
void pushInputBuffer(InputBuffer *inputBuffer) {
  yypush_buffer_state((YY_BUFFER_STATE)inputBuffer->buffer,
                      (yyscan_t)inputBuffer->lexicalAnalyzer->scanner);
}

CompilationStatus pushToken(LexicalAnalyzer *lexicalAnalyzer, Token *token) {
  return (CompilationStatus)yypush_parse((yypstate *)lexicalAnalyzer->parser,
                                         token->label, token->semanticValue,
