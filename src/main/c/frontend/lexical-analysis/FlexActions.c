...
static int _parse_value_and_unit(const char *side, float *outVal,
                                 UnitType *outUnit);
...
CompilationStatus DimensionsLexemeAction() {
  ...
  if (token && token->semanticValue) {
    ...
    if (sscanf(token->lexeme, "%dx%d", &width, &height) == 2 ||
        sscanf(token->lexeme, "%dX%d", &width, &height) == 2) {
      token->semanticValue->dimensions.width = width;
      token->semanticValue->dimensions.height = height;
      token->semanticValue->dimensions.widthUnit = UNIT_PX;
      token->semanticValue->dimensions.heightUnit = UNIT_PX;
    }
  }
  _logTokenAction(__FUNCTION__, token);
  CompilationStatus status = pushToken(_lexicalAnalyzer, token);
...
CompilationStatus SubexpressionLexemeAction() { return IgnoredLexemeAction(); }
